...
def compute_average_effective_flops(single_model_gflops_list, hesitant_net_gflops, activation_rates_pct):
    """
    avg_flops = FLOPs(hesitant_fuzzy routing net) + sum_i( activation_rate_i * FLOPs(model_i) )
    """
    avg_flops = hesitant_net_gflops
    for i, act_pct in enumerate(activation_rates_pct):
        avg_flops += (act_pct / 100.0) * single_model_gflops_list[i]
    return avg_flops
    
def compute_complexity(model, input_size=(1, 3, 256, 256), device='cuda',
                        model_name="", count_trainable_only=False):
    ...
    unsupported = flop_analysis.unsupported_ops()
    ...
    if unsupported:
        ...
        print(f"     OK: no unsupported operators - FLOPs count is complete.")
...
def compute_average_effective_flops(single_model_gflops_list, hesitant_net_gflops, activation_rates_pct):
    """
    محاسبه‌ی FLOPs موثر (میانگین واقعی) با استفاده از نرخ فعال‌سازی هر مدل
    که از model_activation_stats.json به‌دست می‌آید (activation_rates_pct).
    این عدد دقیق‌ترین گزارش برای مقاله است، چون بازتاب رفتار واقعی مدل
    روی دیتاست تست است، نه یک dummy input تکی.

    avg_flops = FLOPs(hesitant_fuzzy) + sum_i( activation_rate_i * FLOPs(model_i) )
    """
    avg_flops = hesitant_net_gflops
    for i, act_pct in enumerate(activation_rates_pct):
        avg_flops += (act_pct / 100.0) * single_model_gflops_list[i]
...
def train_hesitant_fuzzy(ensemble_model, train_loader, val_loader, num_epochs, lr,
    ...
    print("="*70)
    print("Training Fuzzy Hesitant Network (Single GPU)")
    print("="*70)
...
# ================== MAIN FUNCTION ==================
def main():
    parser = argparse.ArgumentParser(description="Optimized Fuzzy Hesitant Ensemble Training (Single GPU)")
    # آرگومان‌های اصلی
    ...
    print(f"Single GPU Mode | Seed: {args.seed}")
    print("="*70)
    ...
    # ==========================================
    #   محاسبه FLOPs و Params (قبل از هرچیز)
    # ==========================================
    ...
    # [1] هر مدل پایه (پروندشده) به‌صورت جداگانه
    complexity_results["single_models"] = {}
    for i, model in enumerate(base_models):
        print(f"\n[Single Model {i+1}] {MODEL_NAMES[i]}:")
        complexity_results["single_models"][MODEL_NAMES[i]] = compute_complexity(
            model, device=device, model_name=MODEL_NAMES[i])

    # [2] کل Ensemble با یک dummy input (worst-case برای همان نمونه)
    print(f"\n[Ensemble] FuzzyHesitantEnsemble (dummy-input, single forward):")
    complexity_results["ensemble_dummy_input"] = compute_complexity(
        ensemble, device=device, model_name="FuzzyHesitantEnsemble")

    # [2.5] فقط شبکه‌ی routing (hesitant_fuzzy) به‌تنهایی
    print(f"\n[Routing Net Only] HesitantFuzzyMembership:")
    complexity_results["hesitant_net_only"] = compute_complexity(
        ensemble.hesitant_fuzzy, input_size=(1, 3, 256, 256), device=device,
        model_name="HesitantFuzzyMembership (routing net)")

    # [3] Worst-case واقعی: اجبار به فعال‌سازی همه مدل‌ها
    print(f"\n[Worst-case] Forcing all base models to be active:")
    orig_cum, orig_hes = ensemble.cum_weight_threshold, ensemble.hesitancy_threshold
    ensemble.cum_weight_threshold = 1.0
    ensemble.hesitancy_threshold = -1.0
    complexity_results["ensemble_worst_case_all_active"] = compute_complexity(
        ensemble, device=device, model_name="FuzzyHesitantEnsemble (all active)")
    ensemble.cum_weight_threshold, ensemble.hesitancy_threshold = orig_cum, orig_hes

    print("="*70)
    with open(os.path.join(args.save_dir, 'complexity_results.json'), 'w') as f:
        json.dump(complexity_results, f, indent=4)
    print(f"Complexity results saved to: {os.path.join(args.save_dir, 'complexity_results.json')}\n")
    trainable = sum(p.numel() for p in ensemble.parameters() if p.requires_grad)
    total = sum(p.numel() for p in ensemble.parameters())
    print(f"Total params: {total:,} | Trainable: {trainable:,} | Frozen: {total-trainable:,}\n")

    # استفاده از دیتالودرهای غیرتوزیع‌شده
    ...
    activation_json_path = os.path.join(args.save_dir, "model_activation_stats.json")
    if os.path.exists(activation_json_path):
        with open(activation_json_path, 'r', encoding='utf-8') as f:
            activation_stats = json.load(f)

        single_gflops_list = [
            complexity_results["single_models"][name]["total_gflops"]
            for name in MODEL_NAMES
        ]
        activation_rates_pct = [
            activation_stats["activation_rates_pct"][name] for name in MODEL_NAMES
        ]
        hesitant_net_gflops = complexity_results["hesitant_net_only"]["total_gflops"]  # <-- عدد واقعی، نه placeholder

        avg_effective_gflops = compute_average_effective_flops(
            single_gflops_list,
            hesitant_net_gflops=hesitant_net_gflops,
            activation_rates_pct=activation_rates_pct
        )
        print(f"\nAverage Effective GFLOPs (based on real activation rates): {avg_effective_gflops:.3f} GFLOPs")
        print(f"  (Routing net overhead: {hesitant_net_gflops:.3f} GFLOPs, always active)")
        complexity_results["average_effective_gflops"] = avg_effective_gflops
